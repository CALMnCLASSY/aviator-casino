...
(function () {
    ...
    function flwGetCurrencyLimits(currency) {
        if (typeof window.dynamicMinDeposit === 'number' && typeof window.dynamicMaxDeposit === 'number') {
            const sym = (typeof window.getCurrencySymbol === 'function') ? window.getCurrencySymbol(currency) : '$';
            return { min: window.dynamicMinDeposit, max: window.dynamicMaxDeposit, symbol: sym };
        }
        return FLW_CURRENCY_LIMITS[currency || 'USD'] || FLW_CURRENCY_LIMITS.USD;
    }
